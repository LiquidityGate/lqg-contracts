...
contract LQGDAOProtocolSettingsMinipool is LQGDAOProtocolSettings, LQGDAOProtocolSettingsMinipoolInterface {

    uint256 constant internal minipoolUserDistributeWindowStart = 90 days;

    constructor(LQGStorageInterface _lqgStorageAddress) LQGDAOProtocolSettings(_lqgStorageAddress, "minipool") {
        version = 3;
    ...
    function setSettingUint(string memory _settingPath, uint256 _value) override public onlyDAOProtocolProposal {
        ...
        if(getBool(keccak256(abi.encodePacked(settingNameSpace, "deployed")))) {
            ...
            if(settingKey == keccak256(abi.encodePacked("minipool.launch.timeout"))) {
                ...
                require(_value >= (lqgDAONodeTrustedSettingsMinipool.getScrubPeriod() + 1 hours), "Launch timeout must be greater than scrub period");
                // >= 12 hours (RPIP-33)

...
contract LQGRewardsPool is LQGBase, LQGRewardsPoolInterface {
    ...
    /// @return uint256 Returns rpl balance of lqg rewards contract
    function getRPLBalance() override public view returns(uint256) {
        ...
        return lqgVault.balanceOfToken("lqgRewardsPool", IERC20(getContractAddress("lqgTokenRPL")));
    }
    ...
    function getPendingRPLRewards() override public view returns (uint256) {
        LQGTokenRPLInterface rplContract = LQGTokenRPLInterface(getContractAddress("lqgTokenRPL"));
    ...
    function getPendingETHRewards() override public view returns (uint256) {
        address lqgSmoothingPoolAddress = getContractAddress("lqgSmoothingPool");
        return lqgSmoothingPoolAddress.balance;
    }
    ...
    function submitRewardSnapshot(RewardSubmission calldata _submission) override external onlyLatestContract("lqgRewardsPool", address(this)) onlyTrustedNode(msg.sender) {
        // Get contracts
        ...
        require(lqgDAOProtocolSettingsNetwork.getSubmitRewardsEnabled(), "Submitting rewards is currently disabled");
        // Validate inputs
    ...
    function executeRewardSnapshot(RewardSubmission calldata _submission) override external onlyLatestContract("lqgRewardsPool", address(this)) {
    ...
    function _executeRewardSnapshot(RewardSubmission calldata _submission) private {
        ...
        if (_submission.treasuryRPL > 0) {
            lqgVault.transferToken("lqgClaimDAO", rplContract, _submission.treasuryRPL);
        }
        // Get the smoothing pool instance
        LQGSmoothingPoolInterface lqgSmoothingPool = LQGSmoothingPoolInterface(getContractAddress("lqgSmoothingPool"));
        // Send deposit pool user's ETH
        if (_submission.userETH > 0) {
            address lqgTokenRETHAddress = getContractAddress("lqgTokenRETH");
            lqgSmoothingPool.withdrawEther(lqgTokenRETHAddress, _submission.userETH);
        }
        ...
        for (uint i = 0; i < _submission.nodeRPL.length; ++i) {
            ...
            LQGRewardsRelayInterface relay;
            { // Scope to prevent stack too deep
            ...
            if (rewardsRPL > 0) {
                ...
                lqgVault.withdrawToken(address(relay), rplContract, rewardsRPL);
            }
            if (rewardsETH > 0) {
